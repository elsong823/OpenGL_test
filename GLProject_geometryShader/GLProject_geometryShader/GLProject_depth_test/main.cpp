...
GLuint boxReflectShaderProgram = 0;
GLuint boxRefractShaderProgram = 0;
...
void InitObject()
{
	skyboxShaderProgram = CMyShader::GetInstance()->CreateShaderProgram("../bin/shaders/sky.vert","../bin/shaders/sky.frag");
	modelShaderProgram = CMyShader::GetInstance()->CreateShaderProgram("../bin/shaders/model.vert", "../bin/shaders/model.frag");
	lineShaderProgram = CMyShader::GetInstance()->CreateShaderProgram("../bin/shaders/line.vert","../bin/shaders/line.geo","../bin/shaders/line.frag");
	CreateSkyBox();
	camera = new CMyCamera(45.0f, WIN_WIDTH, WIN_HEIGHT, glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f));
	myModel = new CMyModel("../bin/res/nanosuit.obj", texSkybox);
}
...
{	
	...
	GLuint shaders[2] = {
		boxReflectShaderProgram,
		boxRefractShaderProgram,
	};
	//draw cubes
	glm::mat4 matModel;
	...
	{
		matModel = glm::mat4();
		matModel = glm::translate(matModel, cubes[i].position);
		matModel = glm::scale(matModel, cubes[i].scale);
		auto shader = shaders[i];
		glUseProgram(shader);
		glUniformMatrix4fv(glGetUniformLocation(shader, "matModel"), 1, GL_FALSE, glm::value_ptr(matModel));
		glUniformMatrix4fv(glGetUniformLocation(shader, "matView"), 1, GL_FALSE, glm::value_ptr(*camera->GetView()));
		glUniformMatrix4fv(glGetUniformLocation(shader, "matProjection"), 1, GL_FALSE, glm::value_ptr(*camera->GetProjection()));
		glUniform3fv(glGetUniformLocation(shader, "eyePos"), 1, glm::value_ptr(*camera->GetPosition()));
		glUniform1i(glGetUniformLocation(shader, "skyBox"), 0);
		//glUniform1i(glGetUniformLocation(boxReflectShaderProgram, "material.texDiffuse"), 0);
		//glUniform1i(glGetUniformLocation(boxReflectShaderProgram, "material.texSpecular"), 1);
		//glUniform1f(glGetUniformLocation(boxReflectShaderProgram, "material.shininess"), cube.material.shininess);
		//glUniform3fv(glGetUniformLocation(boxReflectShaderProgram, "directLight.direction"), 1, glm::value_ptr(dLight.direction));
		//glUniform3fv(glGetUniformLocation(boxReflectShaderProgram, "directLight.ambient"), 1, glm::value_ptr(dLight.ambient));
		//glUniform3fv(glGetUniformLocation(boxReflectShaderProgram, "directLight.diffuse"), 1, glm::value_ptr(dLight.diffuse));
		//glUniform3fv(glGetUniformLocation(boxReflectShaderProgram, "directLight.specular"), 1, glm::value_ptr(dLight.specular));
		//glUniform3fv(glGetUniformLocation(boxReflectShaderProgram, "eyePos"), 1, glm::value_ptr(*camera->GetPosition()));
...
{
	vector<string> pics;
	pics.emplace_back("../bin/res/right.jpg");
	pics.emplace_back("../bin/res/left.jpg");
	pics.emplace_back("../bin/res/top.jpg");
	pics.emplace_back("../bin/res/bottom.jpg");
	pics.emplace_back("../bin/res/back.jpg");
	pics.emplace_back("../bin/res/front.jpg");
	...
	{
		source = SOIL_load_image(pics[i].c_str(), &width, &height, 0, SOIL_LOAD_RGB);
...
void DrawSkyBox()
{
	...
	glm::mat4 matView = glm::mat4(glm::mat3(*camera->GetView()));
	...
	glUniform1i(glGetUniformLocation(skyboxShaderProgram, "cubeTexture"), 0);
...
void DrawModel()
{
	//model_1->SetPos(glm::vec3(0.0f, 4.0f, 0.0f));
	//model_1->Draw(modelShaderProgram, camera, 1);

	//model_1->SetPos(glm::vec3(3.0f, 4.0f, 0.0f));
	//model_1->Draw(modelShaderProgram, camera, 2);

	myModel->Draw(modelShaderProgram, camera, 2);
	myModel->Draw(lineShaderProgram, camera, 0);
}
...
void CreatePoint()
{
	...
	auto pointer = glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
	memcpy(pointer, points, sizeof(points));
	glUnmapBuffer(GL_ARRAY_BUFFER);

	glBindVertexArray(0);
	//pointsShaderProgram = CMyShader::GetInstance()->CreateShaderProgram("../bin/shaders/shader_point.vert","../bin/shaders/shader_point.frag");
	pointsShaderProgram = CMyShader::GetInstance()->CreateShaderProgram("../bin/shaders/shader_point.vert", "../bin/shaders/shader_point.geo","../bin/shaders/shader_point.frag");
}
