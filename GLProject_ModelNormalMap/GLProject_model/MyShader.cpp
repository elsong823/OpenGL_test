...
GLuint CMyShader::CreateShaderProgram(GLchar * _vert, GLchar * _geo, GLchar * frag)
{
	auto vertSource = this->LoadFile(_vert);
	auto vertShader = glCreateShader(GL_VERTEX_SHADER);
	glShaderSource(vertShader, 1, &vertSource, nullptr);
	...
	auto geoSource = this->LoadFile(_geo);
	auto geoShader = glCreateShader(GL_GEOMETRY_SHADER);
	glShaderSource(geoShader, 1, &geoSource, nullptr);
	glCompileShader(geoShader);
	this->CheckShader(geoShader);
